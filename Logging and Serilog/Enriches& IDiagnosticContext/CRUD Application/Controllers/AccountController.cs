          ...
          IdentityResult result= await _usermanager.CreateAsync(user, registerDTO.Password);
            if (result.Succeeded)
            {
                ...
                else
                {
					...
					//so wehave to make it first
					if (await _roleManagaer.FindByNameAsync(nameof(ApplicationRoleEnum.User)) is null)
                    {
                        ApplicationRole UserRole = new ApplicationRole() { Name = ApplicationRoleEnum.User.ToString() };
                       await _roleManagaer.CreateAsync(UserRole);
                    }
                    //add ouruser to Role
