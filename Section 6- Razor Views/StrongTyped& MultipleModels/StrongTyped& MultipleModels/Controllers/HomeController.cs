...
using StrongTyped__MultipleModels.Models;

namespace StrongTyped__MultipleModels.Controllers
{
    public class HomeController : Controller
    {
			...
			List<Person> people = new List<Person>()
	  {
        ...
        public IActionResult MultipleStrongtyped()
        {
            Person prs = new Person() { Name = "John", DateOfBirth = DateTime.Parse("2000-05-06"), PersonGender = Gender.Male }
            Product product = new Product() { ProductId=1, ProductName="fish"};

            WrappedModel ModelofbothMultipleModels = new WrappedModel() { personData=prs, productData=product};
			return View("MultipleModels", ModelofbothMultipleModels);
        }
    }
}
